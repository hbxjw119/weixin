	...
	class Mysql{
		...
		static private function __where($info, $type = 'AND'){
			...
			foreach($info as $row => $value){
				if(empty($where)){
					$where = sprintf("WHERE `%s`='%s'", $row, $link->real_escape_string($value));
				}else{
					$where .= sprintf(" %s `%s`='%s'", $type, $row, $link->real_escape_string($value));
				}
			}
			self::$where = $where;
		}
		...
		public function get($table, $select = '*'){
			...
			if(!($result = $link->query($sql))){
				throw new Exception('Error executing MySQL query: '.$sql.'. MySQL error '.$link->error);
				$data = false;
			}else{
				$data = array();
				self::set('num_rows',$result->num_rows);
